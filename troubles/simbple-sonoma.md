...
- `book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib` string tables include:
...
## Log (append-only)
- 2024-12-03: extracted `fcntl-command` and `fsctl-command` named-argument values from `book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib` (world_id `sonoma-14.4.1-23E224-arm64-dyld-2c0602c5`) and replaced placeholders in `book/tools/sbpl/simbple/src/platform_data/catalina/filters.c` (`F_GETCONFINED`, `F_SETCONFINED`, and `APFSIOC_*` values).
...
- 2024-12-03: removed the ad-hoc `iokit-user-client-class` filter entry from `book/tools/sbpl/simbple/src/platform_data/catalina/filters.c` to align with `book/integration/carton/bundle/relationships/mappings/vocab/filters.json` (SBPL aliases it to `iokit-registry-entry-class`).
...
- 2024-12-03: searched `book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib` for `*ios-sandbox-*` values; found `com.apple.sandbox.system-container`, `com.apple.sandbox.system-group`, and `com.apple.sandbox.executable` strings (partial host evidence) and updated `book/tools/sbpl/simbple/src/scm/sbpl_v1.scm` accordingly. No `*ios-sandbox-executable*` string found.
