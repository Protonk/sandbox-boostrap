...
- Because entitlements are inputs to the platform policy, a process without them can be denied even if its own per-process profile would otherwise allow the operation (see `guidance/Appendix.md` on filter vocabulary).
