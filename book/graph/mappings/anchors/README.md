...
- `anchor_field2_map.json` is derived from `book/evidence/experiments/field2-final-final/probe-op-structure/out/anchor_hits.json`, which is produced by decoding probe and system profiles via `book/api/profile/decoder/` under the canonical tag layouts (`book/evidence/graph/mappings/tag_layouts/tag_layouts.json`).
