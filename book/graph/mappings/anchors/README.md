...
- `book/tests/planes/graph/test_anchor_filter_alignment.py` enforces that every mapped, non‑blocked anchor in `anchor_filter_map.json` (an entry with a `filter_id` and `sources`, `status != "blocked"`) is backed by concrete witnesses in `anchor_hits.json`:
