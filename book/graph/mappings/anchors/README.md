...
- `anchor_field2_map.json` is derived from `book/experiments/probe-op-structure/out/anchor_hits.json`, which is produced by decoding probe and system profiles via `book/api/profile_tools/decoder/` under the canonical tag layouts (`book/graph/mappings/tag_layouts/tag_layouts.json`).
