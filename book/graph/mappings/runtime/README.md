...
Runtime probe outputs that are stable enough to reuse live under `book/evidence/graph/mappings/runtime/`. These are versioned to the Sonoma baseline (23E224) and come from runtime promotion packets (clean launchd channel).
...
- Use `book/evidence/graph/mappings/vocab/ops_coverage.json` and explicit status fields to gauge what is currently runtime-backed vs blocked on this host.
...
- Run `book/graph/mappings/runtime/promote_from_packets.py` (defaults to `packet_set.json`) to rebuild `book/evidence/graph/mappings/runtime_cuts/`, `runtime_story`, `runtime_coverage`, `runtime_callout_oracle`, and `runtime_signatures.json` from promotion packets, and to refresh `promotion_receipt.json`.
...
- Rerun `book/graph/mappings/runtime/generate_lifecycle.py` after updating lifecycle probes in `book/evidence/graph/concepts/validation/out/lifecycle/` to refresh `lifecycle.json` and `lifecycle_traces/`.
