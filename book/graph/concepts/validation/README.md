...
Current status: sandbox-exec-based semantic and lifecycle runs are deferred while the harness is being repaired; static ingestion and vocab mapping are current, and a first CARTON snapshot (see `book/graph/carton/CARTON.json`) is wired up as the frozen IR/mapping web this validation layer feeds.
