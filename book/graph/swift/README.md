...
- **Inputs:** substrate concept markdown, `book/graph/concepts/CONCEPT_INVENTORY.md`, `book/graph/concepts/validation/Concept_map.md`, `book/examples/`, and any mapping JSONs you load here explicitly.
...
- **Make targets:** `make -C book build` runs Python + Swift with module caches pinned to `book/graph/.module-cache`; `make -C book clean` resets SwiftPM/.build/module caches and CI stamps.
