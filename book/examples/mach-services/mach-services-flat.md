...
`lessons.md` tells you what to watch for:

* `mach-lookup` is a **first-class sandbox operation**.

  * SBPL operates on service names via filters like `(global-name "...")`.
* Platform policy can still deny:

  * registrations,
  * lookups,
    even if a per-process profile would appear permissive.
* By trying different service names and watching success/failure, you see how **service strings become policy inputs**.
* Just registering and looking up names is enough to mirror the kernel path Seatbelt protects.

The code is the concrete, runnable side of those bullets.
