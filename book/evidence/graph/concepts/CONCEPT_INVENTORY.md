...
- **Legacy sandbox-exec traces (brittle, but still informative)**
  - `book/evidence/graph/concepts/validation/out/semantic/{metafilter.jsonl,sbpl_params.jsonl,network.jsonl,mach_services.jsonl}` – older logs using sandbox-exec-based harnesses.
