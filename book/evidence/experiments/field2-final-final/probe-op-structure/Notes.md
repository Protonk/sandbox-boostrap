...
- Adjusted `analyze_profiles.py` and `anchor_scan.py` to compute the repository root as three parents up from this experiment directory (so `book.*` imports resolve correctly), then reran both scripts from the repo root. `out/analysis.json` and `out/anchor_hits.json` are now refreshed under the current `book.api.profile.decoder` (which prefers `book/integration/carton/bundle/relationships/mappings/tag_layouts/tag_layouts.json` when present).
...
- Ran `python -m book.api.runtime run --plan book/evidence/experiments/runtime-final-final/suites/field2-probe-op-structure/plan.json --channel launchd_clean --out book/evidence/experiments/runtime-final-final/suites/field2-probe-op-structure/out` → run id `39f84aa5-86b4-466d-b5d9-f510299bbd0a` (status: partial).
...
- Runtime-closure cross-check: `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/5a8908d8-d626-4cac-8bdd-0f53c02af8fe/` file lane shows `/etc/hosts` denied across alias-only, private-only, and both profiles; `/private/etc/hosts` allowed only when explicitly allowed; `/tmp/foo` denied across all profiles. `path_witnesses.json` baseline resolves `/etc/hosts` -> `/private/etc/hosts` and `/tmp/foo` -> `/private/tmp/foo`, and scenario success shows `F_GETPATH_NOFIRMLINK:/System/Volumes/Data/private/etc/hosts` for `/private/etc/hosts`.
- Runtime-closure mach lane run `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/66315539-a0ce-44bf-bff0-07a79f205fea/`: `com.apple.cfprefsd.agent` allowed in baseline and scenario; `com.apple.sandbox-lore.missing` returns `kr=1102` in baseline and scenario, confirming the missing-service control behaves the same without the sandbox.
- Runtime-closure IOKit lane run `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/48086066-bfa2-44bb-877c-62dd1dceca09/`: baseline `IOSurfaceRoot` opens (`open_kr=0`), scenario `sandbox_iokit_probe` reports `open_kr=-536870174` with `EPERM`, giving a discriminating IOKit signal on this host.
- Runtime-closure file matrix run `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/ea704c9c-5102-473a-b942-e24af4136cc8/` (v2 spellings):
...
- Runtime-closure IOKit user-client run `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/6ecc929d-fec5-4206-a85c-e3e265c349a7/`:
...
- Runtime-closure IOKit op-identity micro-matrix runs `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/08887f36-f87b-45ff-8e9e-6ee7eb9cb635/` and `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/33ff5a68-262a-4a8c-b427-c7cb923a3adc/`:
  - Both `iokit-open-user-client` (v2) and `iokit-open` (v4) allow `IOSurfaceRoot` at operation stage, so op identity remains ambiguous.
- Runtime-closure IOKit op-identity tri-matrix run `book/evidence/experiments/runtime-final-final/suites/runtime-closure/out/fae371c2-f2f5-470f-b672-cf0c3e24d6c0/`:
