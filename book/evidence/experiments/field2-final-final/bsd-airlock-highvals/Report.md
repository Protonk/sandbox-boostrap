...
- ✅ First probe pass: compiled/decoded two probes via `run_probes.py` (compiled with `book.api.profile`, decoded with `book.api.profile.decoder` into `out/decode_records.jsonl` / `out/field2_summary.json`).
