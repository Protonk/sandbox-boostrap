...
- Refreshed static join with `PYTHONPATH=$PWD python3 book/tools/policy/ratchet/atlas_static.py`.
...
- Added historical runtime witness carry-forward in `book/tools/policy/ratchet/atlas_runtime.py` so apply-gated runs can still report last-known-good results as `runtime_backed_historical` when available.
...
- `book/tools/policy/ratchet/atlas_static.py` now merges seed-manifest anchors into `anchor_hits` when the anchor map lacks coverage; sysctl-name and xpc-service-name seeds carry seed-manifest anchors to keep static records non-empty.
