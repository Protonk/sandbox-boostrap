...
- From `book/evidence/experiments/node-layout/`:
  - `out/summary.json` – structural summaries of node/layout behavior.
- From `book/evidence/experiments/op-table-operation/`:
...
- Shared tooling: when regenerating alignments, prefer `book/api/profile/` (`op-table` CLI or `book.api.profile.op_table` in Python) instead of extending `update_alignment.py`.
...
- `book/evidence/experiments/op-table-operation/out/summary.json`, `op_table_map.json`, and `op_table_signatures.json` as upstream structural inputs.
- `book/evidence/experiments/op-table-vocab-alignment/out/op_table_vocab_alignment.json` capturing per-profile operations, op-table buckets, and (when present) vocabulary IDs and versions.
...
1. **Read the upstream experiments**
   - Skim `book/evidence/experiments/node-layout/ResearchReport.md` and `Plan.md`.
   - Skim `book/evidence/experiments/op-table-operation/ResearchReport.md` and `Plan.md`.
...
2. **Confirm current alignment state**
   - Inspect `book/evidence/experiments/op-table-vocab-alignment/out/op_table_vocab_alignment.json` to see:
