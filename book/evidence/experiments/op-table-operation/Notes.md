...
- Initialized a new experiment under `book/evidence/experiments/op-table-operation` to map SBPL operation names to op-table entry indices in compiled profiles.
...
- Updated: refreshed the promoted mapping snapshots under `book/graph/mappings/op_table/` from the experiment outputs using `book/graph/mappings/op_table/generate_op_table_mappings.py` (the curated mapping set excludes `v12_runtime_probe`, which remains experiment-local).
