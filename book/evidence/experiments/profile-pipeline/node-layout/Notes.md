...
- Baseline blob: `book/evidence/syncretic/validation/fixtures/blobs/sample.sb.bin` (583 bytes). Heuristic slices:
...
- Created minimal SBPL variants under `book/evidence/experiments/profile-pipeline/node-layout/sb/`:
...
- Persisted tooling/output:
  - Added `book/evidence/experiments/profile-pipeline/node-layout/analyze.py` to compile SBPL variants, emit blobs, and write `out/summary.json` (lengths, op entries, section lengths, stride stats, tail records).
  - Running with `PYTHONPATH=. python3 book/evidence/experiments/profile-pipeline/node-layout/analyze.py` regenerates build blobs and structured summaries for future analysis.
