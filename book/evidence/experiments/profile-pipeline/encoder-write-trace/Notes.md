...
- `nm -m` on the extracted libsandbox image (`book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib`) shows `_sb_mutable_buffer_write` as non-external; `otool -Iv` shows no indirect-symbol entry for it.
- `xcrun dyld_info -exports /usr/lib/libsandbox.1.dylib` and `-imports` do not list `_sb_mutable_buffer_write` (no matches).
- `xcrun dyld_info -exports book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib` reports segment load commands out of order, so dyld_info evidence uses the `/usr/lib` image as a fallback.
...
- Attempted a non-shared-cache helper by setting `SBPL_SANDBOX_PATH=book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib`; `ctypes.CDLL` failed with a code-signature error ("Trying to load an unsigned library").
...
- Ran `book/tools/sbpl/encoder_write_trace_analyze.py` and `book/tools/sbpl/encoder_write_trace_check.py`; outputs written to `out/trace_analysis.json` and `out/trace_join_check.json` (join check skipped network pairs because only `baseline_allow_all` is present in the trace manifest).
...
- Extended `book/tools/sbpl/encoder_write_trace_analyze.py` to align gapped coverage to a blob window and emit `witnessed_ranges` vs `hole_ranges`; `book/tools/sbpl/encoder_write_trace_check.py` now labels offsets as witnessed vs inferred when coverage is gapped (still skips network pairs without trace coverage).
...
- Ran the domain pair (`domain_af_inet`, `domain_af_system`) using `inputs_domain_pair.json` and re-ran `book/tools/sbpl/encoder_write_trace_analyze.py` + `book/tools/sbpl/encoder_write_trace_check.py`; one network diff was checked and both sides reported missing coverage at offset 410.
- Extended `inputs_domain_pair.json` with the type pair (`type_sock_stream`, `type_sock_dgram`) and re-ran `run_trace.py` + `book/tools/sbpl/encoder_write_trace_analyze.py` + `book/tools/sbpl/encoder_write_trace_check.py`; two network diffs were checked and both pairs still missed offset 410 (outside the traced 21‑byte window).
- Updated `book/tools/sbpl/encoder_write_trace_check.py` to prefer gapped alignment windows when they provide wider coverage than the best subset match; reran the join check and both domain/type pairs now report witnessed coverage at offset 410.
- Expanded the input set with the proto pair (`proto_tcp`, `proto_udp`) and re-ran `run_trace.py` (using `--allow-zero-hits` after a one-off SIGSEGV), then re-ran `book/tools/sbpl/encoder_write_trace_analyze.py` and `book/tools/sbpl/encoder_write_trace_check.py`; three network pairs now report witnessed coverage at offset 410.
...
- Added a compile-string path and compile params handling in `run_trace.py` (compile via `book.api.profile.compile`), then ran `run_trace.py --inputs book/evidence/experiments/profile-pipeline/encoder-write-trace/inputs_params.json --mode triage`.
...
- Ran `run_trace.py --inputs book/evidence/experiments/profile-pipeline/encoder-write-trace/inputs_params.json --mode hw_breakpoint` and re-ran `book/tools/sbpl/encoder_write_trace_analyze.py` for the expanded set. Baseline allow_all file vs string matched trace counts (214 each) and window `[0,416)` with a `[394,400)` hole. Adding an unused param kept the window but reduced records (193).
