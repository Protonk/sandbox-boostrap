...
- Updated: refreshed the promoted mapping snapshots under `book/integration/carton/bundle/relationships/mappings/op_table/` from the experiment outputs using `book/graph/mappings/op_table/generate_op_table_mappings.py` (the curated mapping set excludes `v12_runtime_probe`, which remains experiment-local).
