...
This report distinguishes what is directly witnessed from what is inferred.
...
- **Operations:** `file-read*` and `file-write*` (plus a read-only `/etc/hosts` family).
- **Profile pattern:** per-family tri-profiles (alias-only, canonical-only, both) plus a “data spelling only” variant for the `/var/tmp` discriminator family.
...
- `out/derived/decode_tmp_profiles.json`: decode summary for the compiled blobs under `out/<run_id>/sb_build/*.sb.bin` (see caveat below).
...
- **`/etc` read-only**
  - `vfs_etc_private_only` and `vfs_etc_both` allow `/private/etc/hosts` but deny `/etc/hosts`.
...
3. **`/var/tmp`, `/etc`, and intermediate symlink-in-path requests are not explained by “simple rewrite to /private/…” in this suite.**
   - `/var/tmp/canon`, `/etc/hosts`, and the intermediate-symlink-in-path request are denied even under profiles that allow both the alias spelling and the corresponding `/private/...` spelling.
   - Baseline-lane witnesses show these alias spellings resolve to `/private/...` outside the sandbox, but the sandboxed denials imply either (a) an additional authorization step during traversal (for example, symlink component traversal), or (b) a match spelling that is not captured by the simple `/private/...` witness.
   - These families remain **partial**: the suite records the mismatch but does not claim a single decisive match spelling that explains the denial. Upgrading “deny-side spelling” claims here requires a separate, sandbox-originated reporting witness (see `Plan.md`).
...
- **Status:** host-bound runtime evidence for the specific families above.
  ...
  - `/var/tmp`, `/etc`, and intermediate symlink-in-path outcomes are **partial/under exploration**.
