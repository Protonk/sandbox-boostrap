...
Metadata canonicalization is handled by `book/evidence/experiments/runtime-final-final/suites/metadata-runner/`.

## Relationship to existing work

Upstream inputs:

- **Tag and layout bedrock:** `book/evidence/graph/mappings/tag_layouts/tag_layouts.json` (status: ok, from `tag-layout-decode`).
- **Anchor structure:** `book/evidence/graph/mappings/anchors/anchor_filter_map.json` (guarded by `book/tests/planes/graph/test_anchor_filter_alignment.py`) and anchor placement notes from `book/evidence/experiments/field2-final-final/probe-op-structure/Report.md`.
...
Profiles live under `book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/sb/` and follow a tri-profile pattern (alias-only, canonical-only, both) per variant family:
...
- `python -m book.api.runtime plan-build --template vfs-canonicalization --out book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization --overwrite` keeps `plan.json` and `registry/` in sync (use `--write-expected-matrix` only for a static snapshot; runtime runs emit run-scoped expected_matrix bundles).
- `python book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/prepare_fixtures.py` prepares host fixtures under `/private/tmp` and `/private/var/tmp`.
- `python -m book.api.runtime run --plan book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/plan.json --channel launchd_clean` emits a committed bundle under `out/<run_id>/` and updates `out/LATEST`.
- `python -m book.api.runtime emit-promotion --bundle book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/LATEST --out book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/promotion_packet.json --require-promotable` (when promotion is intended).
- `python book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/derive_outputs.py --bundle book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out --out-dir book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/derived` writes derived summaries (`runtime_results.json`, `decode_tmp_profiles.json`, `mismatch_summary.json`) that include `(run_id, artifact_index digest)` and point back to the bundle.
...
- `out/LATEST/expected_matrix.json` – object with profile-scoped probe expectations:
  ...
  ```jsonc
  {
    ...
    "profiles": {
      "vfs_tmp_only": {
        "blob": "book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/<run_id>/runtime_profiles/vfs_tmp_only.vfs_tmp_only.runtime.sb",
        "mode": "sbpl",
        "family": "vfs_tmp",
...
- `out/derived/runtime_results.json` – derived bundle summary:
  ...
  ```jsonc
  {
    ...
    "world_id": "sonoma-14.4.1-23E224-arm64-dyld-2c0602c5",
    "bundle": {
      "bundle_dir": "book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/<run_id>",
      "run_id": "<run_id>",
      "artifact_index": "book/evidence/experiments/runtime-final-final/suites/vfs-canonicalization/out/<run_id>/artifact_index.json",
