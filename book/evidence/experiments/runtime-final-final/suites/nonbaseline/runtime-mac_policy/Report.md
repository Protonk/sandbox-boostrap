...
4) Align to static IR (asymmetric, one-way):
   - For each `mpc_ops`, map entry pointers to image/segment/offset and compare a representative subset of indices against `book/evidence/graph/mappings/op_table/op_table_map.json`.
