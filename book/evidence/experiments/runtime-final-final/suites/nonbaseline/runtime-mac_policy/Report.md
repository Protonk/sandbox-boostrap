...
4) Align to static IR (asymmetric, one-way):
   - For each `mpc_ops`, map entry pointers to image/segment/offset and compare a representative subset of indices against `book/integration/carton/bundle/relationships/mappings/op_table/op_table_map.json`.
...
- Tooling: `capture.py` (dynamic DTrace wrapper) and `normalize.py` (raw → schema JSON); sanity-checked by experiment-local `book/evidence/experiments/runtime-final-final/suites/nonbaseline/runtime-mac_policy/test_normalize.py` (not run by `make -C book test`).
