...
- Structural cross-checks for anchors and decoder fields moved to `book/evidence/experiments/field2-final-final/metadata-runner/` (field2-focused).
...
- Unified runtime CLI run: `launchd_clean` bootstrap failed (launchctl exit 5); direct channel run succeeded and produced a committed bundle under `book/evidence/experiments/runtime-final-final/suites/metadata-runner/out/<run_id>/`.
