...
```sh
python3 book/evidence/experiments/lifecycle-lockdown/run_lockdown.py --out book/evidence/experiments/lifecycle-lockdown/out
```
...
SANDBOX_LORE_PREFLIGHT_FORCE=1 python3 -m book.api.runtime run \
  --plan book/evidence/experiments/lifecycle-lockdown/plan.json \
  --channel launchd_clean \
  --out book/evidence/experiments/lifecycle-lockdown/out/runtime/launchd_clean_force
```
