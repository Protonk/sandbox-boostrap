...
- Runtime execution via `python -m book.api.runtime run --plan book/evidence/experiments/runtime-adversarial/plan.json --channel launchd_clean`.
- Expectation wiring patterned after `book/evidence/experiments/sbpl-graph-runtime`.
...
- Plan/registry files are generated from the runtime template:
  `python -m book.api.runtime plan-build --template runtime-adversarial --out book/evidence/experiments/runtime-adversarial --overwrite` (plan-build skips expected_matrix.json by default; use `--write-expected-matrix` for a static snapshot).
