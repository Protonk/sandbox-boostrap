...
> Archived experiment scaffold. Canonical mapping: `book/evidence/graph/mappings/system_profiles/digests.json`.

## Purpose
Produce stable digests for curated system profile blobs (for example, `airlock`, `bsd`, `sample`) using the current decoder. Each digest captures op-table buckets, tag counts, literal samples, and basic section info, and is published at `book/evidence/graph/mappings/system_profiles/digests.json` for reuse across the book.
...
- Output: digest JSON at `book/evidence/graph/mappings/system_profiles/digests.json` with build metadata and provenance notes.
...
- **Current status**
  ...
  - Normalized digest published to `book/evidence/graph/mappings/system_profiles/digests.json` with host metadata, op-table entries, node/tag counts, literal sample, sections, and validation flags for `airlock`, `bsd`, and `sample`.
...
- **3) Publish stable artifact**
  - Wrote the curated digest to `book/evidence/graph/mappings/system_profiles/digests.json` with version/build metadata.
...
3. **Publish and guard**
   - Normalize and publish the digest under `book/evidence/graph/mappings/system_profiles/digests.json` with updated host metadata.
...
- Published digest `book/evidence/graph/mappings/system_profiles/digests.json` with host metadata, op-table entries, node/tag counts, literal samples, and section offsets.
