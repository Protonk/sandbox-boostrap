...
- Updated `harvest_field2.py` to treat the third slot explicitly as `filter_arg_raw` with derived `field2_hi = raw & 0xC000` and `field2_lo = raw & 0x3FFF`, and to track per-tag counts. Inventory now ingests `book/evidence/experiments/field2-final-final/probe-op-structure/sb/build` profiles alongside the local probes and system blobs; refreshed output lives at `out/field2_inventory.json`.
...
- Ran `kernel_field2_mask_scan` twice on the 14.4.1 project:
  ...
  - Full-program with masks 0x3fff/0x4000/0xc000/0x00ff/0xff00: no hits. Output at `book/evidence/dumps/ghidra/out/14.4.1-23E224/kernel-field2-mask-scan/mask_scan.json`.
- Ran `kernel_imm_search` on key field2 constants across the full KC: 0xa00 (flow-divert 2560), 0x4114 (bsd tail hi-bit), and 0x2a00 (airlock high). All returned zero hits. Outputs under `book/evidence/dumps/ghidra/out/14.4.1-23E224/kernel-imm-search/`. These negatives suggest the constants are not present as plain immediates; evaluator likely derives flags via other arithmetic or indirect tables.
...
- Output at `book/evidence/dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/field2_evaluator.json`: ~60k instructions, 897 functions. Heuristic picked `__read24` at `fffffe000b410ee4` (loads a halfword + byte with bounds checks) as the smallest widely-called ldrb+ldrh helper; callers include `_eval` at `fffffe000b40d698`, `_populate_syscall_mask`, and `_check_syscall_mask_composable`. The dumped helper/evaluator disasm lives in `helper.txt` / `evaluator.txt` alongside the JSON.
...
- Refined the headless helper hunt (`book/api/ghidra/scripts/find_field2_evaluator.py`): the stricter filter now lands on `__read16` at `fffffe000b40fa1c` as the small u16 reader. Disasm in `book/evidence/dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/helper.txt` shows bounds checks and a plain `ldrh/strh`; no bit tests or masks on the payload. The script also auto-dumps `_eval` to `eval.txt`.
...
- Refreshed `harvest_field2.py` and `unknown_focus.py` outputs. Highlights:
  ...
  - Updated artifacts: `out/field2_inventory.json`, `out/unknown_nodes.json`, Ghidra outputs under `book/evidence/dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/`.
...
- Next possible solutions:
  ...
  - If headless continues to balk, fall back to an interactive Ghidra session to export caller disassembly, then park the dumps under `book/evidence/dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/callers/`.
...
- Headless run succeeded; dumped caller disassembly to `book/evidence/dumps/ghidra/out/14.4.1-23E224/find-field2-evaluator/read16_callers.txt` using a Jython script (`/tmp/dump_read16_callers.py`) that iterates functions by name.
