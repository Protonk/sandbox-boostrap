...
> Archived experiment scaffold. Canonical corpus lives under `book/evidence/graph/concepts/validation/golden_corpus/`.
...
- How do decoder outputs align with the `book.api.profile` inspector and op-table summaries on those blobs?
...
2. Run decoder and `book.api.profile` inspectors on each blob; store raw header/section slices and decoded JSON in `book/evidence/graph/concepts/validation/golden_corpus/`.
3. Build a consolidated `corpus_summary.json` with key signals and the tag-layout digest used.
4. Keep the validation job `experiment:golden-corpus` aligned with the manifest/summary under `book/evidence/graph/concepts/validation/golden_corpus/`.
