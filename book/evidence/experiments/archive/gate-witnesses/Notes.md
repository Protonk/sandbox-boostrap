...
- Witness generation (outside harness sandbox):
  ...
  - `python3 book/tools/preflight/preflight.py minimize-gate --input /System/Library/Sandbox/Profiles/blastdoor.sb --out-dir book/evidence/experiments/gate-witnesses/out/witnesses/blastdoor --confirm 10`
  - `python3 book/tools/preflight/preflight.py minimize-gate --input /System/Library/Sandbox/Profiles/com.apple.CoreGraphics.CGPDFService.sb --out-dir book/evidence/experiments/gate-witnesses/out/witnesses/com.apple.CoreGraphics.CGPDFService --confirm 10`

- Derived-only summaries:
  - `python3 book/evidence/experiments/gate-witnesses/summarize_features.py`

- Compile-vs-apply fork + micro-variant matrix (outside harness sandbox):
  - `python3 book/evidence/experiments/gate-witnesses/compile_vs_apply.py`

- Entitlement scan (codesign; host-local):
  - `python3 book/evidence/experiments/gate-witnesses/scan_entitlements.py`

- Message-filter xref summary (Ghidra + dyld slice string presence):
  - `python3 book/evidence/experiments/gate-witnesses/message_filter_xrefs.py`
...
- Less permissive control pass (non-`--yolo`) airlock run:
  - `python3 book/tools/preflight/preflight.py minimize-gate --input /System/Library/Sandbox/Profiles/airlock.sb --out-dir book/evidence/experiments/gate-witnesses/out/witnesses/airlock --confirm 10`
  - Result: minimal failing still hits apply-stage `EPERM` (10/10 confirmations), but `confirm.passing_neighbor` is null (no passing neighbor confirmed); preserved as `book/evidence/experiments/gate-witnesses/out/witnesses/airlock/run.non_yolo.json`.
