...
- IOSurface call-shape instrumentation:
  - `book/evidence/experiments/runtime-closure/harness/iokit_call_interpose.c` via DYLD_INSERT_LIBRARIES caused a SIGSEGV when running `iokit_probe` and `iosurface_trace`; no output captured (`out/iosurface_call_trace/interpose_stderr.txt` empty).
