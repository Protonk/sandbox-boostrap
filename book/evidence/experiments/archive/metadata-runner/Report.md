...
- Keep metadata canonicalization evidence separate from data read/write behavior (see `book/evidence/experiments/vfs-canonicalization/Report.md`).
...
python -m book.api.runtime run \
  --plan book/evidence/experiments/metadata-runner/plan.json \
  --channel launchd_clean \
  --out book/evidence/experiments/metadata-runner/out
```
...
python -m book.api.runtime emit-promotion \
  --bundle book/evidence/experiments/metadata-runner/out/LATEST \
  --out book/evidence/experiments/metadata-runner/out/promotion_packet.json
```
...
## Evidence & artifacts
- Plan: `book/evidence/experiments/metadata-runner/plan.json`.
- Registry: `book/evidence/experiments/metadata-runner/registry/probes.json`, `book/evidence/experiments/metadata-runner/registry/profiles.json`.
- Runtime bundle authority: `book/evidence/experiments/metadata-runner/out/<run_id>/artifact_index.json`.
- Normalized events: `book/evidence/experiments/metadata-runner/out/<run_id>/runtime_events.normalized.json`.
- Raw results: `book/evidence/experiments/metadata-runner/out/<run_id>/runtime_results.json`.
- SBPL sources: `book/evidence/experiments/metadata-runner/sb/metadata_*.sb`.
- Optional promotion packet: `book/evidence/experiments/metadata-runner/out/promotion_packet.json`.
