...
- Tooling: `capture.py` (dynamic DTrace wrapper) and `normalize.py` (raw → schema JSON); sanity-checked by experiment-local `book/evidence/experiments/runtime-mac_policy/test_normalize.py` (not run by `make -C book test`).
