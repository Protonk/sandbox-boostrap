...
- Non-system target attempt: built `book/evidence/experiments/dtrace-testing/targets/sleep_loop` and attached via `dtrace -p` without sudo; stderr reported `DTrace requires additional privileges` and no events were captured. Stderr saved in `out/meta/dtrace_non_system.stderr`.
