...
## Quick commands
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario inventory`
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario matrix`
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmarks`
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario downloads_rw`
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client`
- `python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_op_groups`
...
- Keep EntitlementJail outputs under `book/evidence/experiments/entitlement-diff/out/ej/`.

## EntitlementJail 1.x inventory run
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario inventory
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario inventory (escalated)
- Result: inventory.json written with 11 commands; all exit_code=0.
- Status: ok

## Matrix run (initial failure)
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario matrix (escalated)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario matrix (escalated)
- Result: run-matrix exit_code=0; outputs copied from container matrix/latest into book/evidence/experiments/entitlement-diff/out/ej/matrix/baseline.
- Status: ok

## Bookmarks run (initial failure)
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmarks (escalated)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmarks (escalated)
- Result: runs completed; minimal bookmark_make returns rc=1 with normalized_outcome=bookmark_make_failed and service_refusal=entitlement_missing_bookmarks_app_scope (expected entitlement gap). Log files copied into book/evidence/experiments/entitlement-diff/out/ej/logs.
- Status: ok (partial runtime evidence; deny evidence depends on log contents)

## Downloads RW capture
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario downloads_rw (escalated)
...
## Net client capture
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client (escalated)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario evidence (escalated)
- Result: verify-evidence and inspect-macho commands exit_code=0; bundle-evidence copied from container path into book/evidence/experiments/entitlement-diff/out/ej/evidence/latest.
- Status: ok

## Matrix groups capture
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario matrix_groups --ack-risk fully_injectable (escalated)
- Result: baseline/debug/inject/jit groups exit_code=0 with copied outputs under book/evidence/experiments/entitlement-diff/out/ej/matrix/<group>/.
- Status: ok

## Probe families (initial fs_xattr failure)
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario probe_families (escalated)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmark_roundtrip (escalated)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario downloads_rw
- Result: log stream files written under `book/evidence/experiments/entitlement-diff/out/ej/logs/`; observer reports written under `book/evidence/experiments/entitlement-diff/out/ej/logs/observer/`; no sandbox deny lines observed for downloads_rw runs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmarks
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario probe_families
- Result: log stream and observer reports captured; no sandbox deny lines observed for userdefaults/fs_xattr/fs_coord probes.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmark_roundtrip
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_attach
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed in wait_attach logs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_timeout_matrix
- Result: log stream captured; no sandbox deny lines observed in wait_timeout logs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_path_class
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_multi_trigger
- Result: log stream captured; no sandbox deny lines observed in wait_multi logs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_probe_wait
- Result: log stream captured; no sandbox deny lines observed in wait_probe logs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_hold_open
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_create
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_interval
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario attach_holdopen_default
- Result: log stream + observer captured for capabilities_snapshot; no sandbox deny lines observed in attach_default logs.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario matrix_groups --ack-risk fully_injectable
- Result: matrix outputs refreshed under `book/evidence/experiments/entitlement-diff/out/ej/matrix/<group>/`; run-matrix does not request log capture, so no new deny evidence for this run.
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_op_groups
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client
- Result: observer report now includes `plan_id`, `row_id`, and `correlation_id` (verified in `book/evidence/experiments/entitlement-diff/out/ej/logs/observer/net_client.minimal.tcp_connect.log`).
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario wait_path_class (path-class only)
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario health_check_profile
- Result: health-check succeeds for both minimal and debuggable with exit_code=0.
- Status: ok

## run-matrix --out
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario run_matrix_out
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bundle_evidence_out
...
- Command: PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario quarantine_lab
...
- Command: `EJ_LOG_MODE=path_class PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client`
...
- Commands: `PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario probe_families` and `PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario bookmark_roundtrip`
