...
```
PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_client
```

Run cross-profile network probes:

```
PYTHONPATH=. python book/evidence/experiments/entitlement-diff/run_entitlementjail.py --scenario net_op_groups
```

## Adding new probes (model pattern)
1) Add a scenario in `book/evidence/experiments/entitlement-diff/ej_scenarios.py`.
