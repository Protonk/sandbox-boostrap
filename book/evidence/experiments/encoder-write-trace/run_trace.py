...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument(
        "--inputs",
        type=Path,
        default=Path("book/evidence/experiments/encoder-write-trace/inputs.json"),
        help="Input list (repo-relative)",
    )
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path("book/evidence/experiments/encoder-write-trace/out"),
    ...
    build_script = ensure_absolute(Path("book/evidence/experiments/encoder-write-trace/harness/build_interposer.sh"), repo_root)
    interposer = ensure_absolute(Path("book/evidence/experiments/encoder-write-trace/out/interposer/sbpl_trace_interpose.dylib"), repo_root)
    if not args.skip_build:
        subprocess.check_call([str(build_script)], cwd=repo_root)

    compile_script = ensure_absolute(Path("book/evidence/experiments/encoder-write-trace/compile_one.py"), repo_root)
