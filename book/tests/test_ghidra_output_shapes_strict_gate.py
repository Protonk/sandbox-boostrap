    ...
    def test_ghidra_strict_shapes_are_required(self):
        ...
        for entry in manifest.get("entries", []):
            ...
            if entry.get("required") is not True:
                failures.append(f"strict entry not required: {entry.get('name')}")
                continue
            if entry.get("output_path") not in full_outputs:
                failures.append(f"strict entry missing from manifest.json: {entry.get('name')}")
                continue
            ...
            if not snapshot_path.exists() or snapshot_path.stat().st_size == 0:
                failures.append(f"strict snapshot missing or empty: {entry.get('name')}")
                continue
