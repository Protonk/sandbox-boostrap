...
pytest plugin via `pytest.ini` (`-p book.tests.support`).
...
- "Last run only" artifact emission under `book/tests/out/`.
...
def _reset_artifacts_root(artifacts_root: Path) -> None:
    ...
    # - users can reliably share `book/tests/out/` as "the last run"
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    # *and* in each worker. We therefore only wipe `book/tests/out/` in the
...
def _run_cmd(
    ...
    *,
    ...
    result = subprocess.run(
        argv,
        # Always capture output so tests can attach it to `book/tests/out/` and
...
def run_cmd(artifact_dir: Path):
    ...
    # runs automatically leaves behind a bundle in `book/tests/out/<nodeid>/`.
