...
`minimize-gate` is contract-driven: it executes candidates via `book/tools/sbpl/wrapper/wrapper` and classifies outcomes using tool markers parsed by `book/api/runtime/core/contract.py` (not stderr substrings).
