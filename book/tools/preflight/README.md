...
`minimize-gate` is contract-driven: it executes candidates via `book/api/SBPL-wrapper/wrapper` and classifies outcomes using tool markers parsed by `book/api/runtime_tools/runtime_contract.py` (not stderr substrings).
