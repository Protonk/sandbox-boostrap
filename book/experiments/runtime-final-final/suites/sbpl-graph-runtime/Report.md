...
- Parameterized SBPL (mapped): compile-time params-handle compilation is now validated for a minimal `(param ...)` profile, and the golden-triple matrix includes a blob-mode runtime witness (`runtime:param_deny_root_ok`) that denies `file-read*` under a parameterized ROOT. Deny-default + exec-based blob probes remain brittle (SIGABRT) for now, so blob-mode parameterization uses `allow default` as the stable carrier.
...
- Status remains `partial` by design; do not treat it as flaky. The same `/tmp` → `/private/tmp` behavior is documented in `book/experiments/runtime-final-final/suites/vfs-canonicalization/Report.md`.
