...
- Output location: run-scoped bundles under `book/experiments/runtime-final-final/suites/runtime-checks/out/<run_id>/` (resolve via `out/LATEST`).
...
python -m book.api.runtime run \
  --plan book/experiments/runtime-final-final/suites/runtime-checks/plan.json \
  --channel launchd_clean \
  --out book/experiments/runtime-final-final/suites/runtime-checks/out
```
...
- **3) Run runtime checks**
  - Plan execution runs via `python -m book.api.runtime run --plan book/experiments/runtime-final-final/suites/runtime-checks/plan.json --channel launchd_clean` and writes bundle artifacts under `out/<run_id>/`.
...
3. **Run runtime checks**
   - Run plan-based probes via `python -m book.api.runtime run --plan book/experiments/runtime-final-final/suites/runtime-checks/plan.json --channel launchd_clean` and read results from `out/LATEST/runtime_results.json`.
...
## Evidence & artifacts
- Probe matrix in `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/expected_matrix.json` describing profiles, probes, and expected outcomes.
- Runtime results in `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/runtime_results.json` and `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/runtime_events.normalized.json`.
- Clean-channel manifests: `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/run_manifest.json` (provenance bundle) and `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/apply_preflight.json` (sandbox_check self check).
- Sandbox_check callouts: `book/experiments/runtime-final-final/suites/runtime-checks/out/LATEST/runtime_results.json` includes `seatbelt_callouts` markers for file/mach probes (oracle lane only).
- Runtime execution via `python -m book.api.runtime run --plan book/experiments/runtime-final-final/suites/runtime-checks/plan.json --channel launchd_clean`; shims live in `book/api/runtime/native/sandbox_runner`, probes in `book/api/runtime/native/probes`, wrapper in `book/tools/sbpl/wrapper`.
