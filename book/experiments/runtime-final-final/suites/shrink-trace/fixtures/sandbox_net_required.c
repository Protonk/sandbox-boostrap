// sandbox_net_required.c
// Deterministic workload with required network allowance (serverless).
//
// Required operations:
//   - read /etc/hosts
...
//   - connect to 127.0.0.1:2000 must be allowed:
//       * EPERM => failure (sandbox deny)
//       * ECONNREFUSED => success (allowed, no listener)
//
// Exit code:
...
static void require_network(int rc, int err, const char *what) {
...
int main(void) {
  ...
  if (ofd >= 0) {
    const char *msg = "hello from sandbox_net_required\n";
    ssize_t wn = write(ofd, msg, strlen(msg));
    require_(wn == (ssize_t)strlen(msg), "write(out/hello.txt)");
    close(ofd);
  }

  // Required network: connect must be allowed (ECONNREFUSED is OK).
  ...
  if (s >= 0) {
    ...
    require_network(rc, errno, "connect(127.0.0.1:2000)");
    close(s);
  } else {
    require_(0, "socket(AF_INET,SOCK_STREAM)");
  }
