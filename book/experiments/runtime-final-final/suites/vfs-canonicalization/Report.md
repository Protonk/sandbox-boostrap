...
python -m book.api.runtime run \
  --plan book/experiments/runtime-final-final/suites/vfs-canonicalization/plan.json \
  --channel launchd_clean \
  --out book/experiments/runtime-final-final/suites/vfs-canonicalization/out
```
...
- **Harness:**
  - Plan-data generated via `python -m book.api.runtime plan-build --template vfs-canonicalization --out book/experiments/runtime-final-final/suites/vfs-canonicalization --overwrite` (plan-build skips expected_matrix.json by default; use `--write-expected-matrix` for a static snapshot).
  - Runtime execution via `python -m book.api.runtime run --plan book/experiments/runtime-final-final/suites/vfs-canonicalization/plan.json --channel launchd_clean` (bundle under `out/<run_id>/`, `out/LATEST` points to the most recent committed run).
  - Derived summaries via `python book/experiments/runtime-final-final/suites/vfs-canonicalization/derive_outputs.py --bundle book/experiments/runtime-final-final/suites/vfs-canonicalization/out --out-dir book/experiments/runtime-final-final/suites/vfs-canonicalization/out/derived`.
  - Structural decode via `book/api/profile/decoder/` using bundle blobs (driven by the derive step).
  - Fixture preparation via `python book/experiments/runtime-final-final/suites/vfs-canonicalization/prepare_fixtures.py` (host-local paths under `/private/tmp` and `/private/var/tmp`).
- **Outputs:**
...
- **Operation coverage (writes).**
  ...
  - Metadata canonicalization is covered by `book/experiments/runtime-final-final/suites/metadata-runner/Report.md` and is out of scope for this experiment.
