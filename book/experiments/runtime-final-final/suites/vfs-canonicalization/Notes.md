...
- Metadata canonicalization is now handled by `book/experiments/runtime-final-final/suites/metadata-runner/`; metadata probes removed from this suite’s matrix.
- Re-ran `python book/experiments/runtime-final-final/suites/vfs-canonicalization/run_vfs.py` with `SANDBOX_LORE_SEATBELT_CALLOUT=1`; refreshed `out/` artifacts. Read/write patterns unchanged.
