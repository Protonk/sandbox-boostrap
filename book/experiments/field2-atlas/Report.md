...
- Field2 1 (`mount-relative-path`): Anchored via `/etc/hosts` and present in `sys:sample` tag 8; runtime scenario `adv:path_edges:allow-subpath` is deny where expected allow. The mismatch is captured via the promotion packet’s mismatch exports with baseline/oracle/normalization controls and labeled `path_normalization_sensitivity`.
