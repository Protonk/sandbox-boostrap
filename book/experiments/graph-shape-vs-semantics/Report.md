...
- Harness: `book/experiments/runtime-adversarial/run_adversarial.py` using the shared `sandbox_runner` / `sandbox_reader`.
- Profiles:
...
- Inputs: `runtime-adversarial` expected/runtime matrices in `book/experiments/runtime-adversarial/out/`.
...
1. **Run adversarial probes**
   ...
   - Produces per-profile, per-probe runtime results in `runtime_results.json` (including expectation IDs, allow/deny, match flags, errno, commands).
