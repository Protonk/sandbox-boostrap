...
- Harness: `python -m book.api.runtime run --plan book/experiments/runtime-adversarial/plan.json --channel launchd_clean` (bundle outputs under `book/experiments/runtime-adversarial/out/LATEST/`).
- Profiles:
...
- Inputs: `runtime-adversarial` expected/runtime matrices in `book/experiments/runtime-adversarial/out/LATEST/`.
- Outputs (this suite): `book/experiments/graph-shape-vs-semantics/out/graph_shape_semantics_summary.json`, generated by `summarize_struct_variants.py`.
...
1. **Run adversarial probes**
   ...
   - Produces per-profile, per-probe runtime results in `book/experiments/runtime-adversarial/out/LATEST/runtime_results.json` (including expectation IDs, allow/deny, match flags, errno, commands).

2. **Compare variants**
   - Command: `python book/experiments/graph-shape-vs-semantics/summarize_struct_variants.py`.
   - This script:
   ...
   - Output: `out/graph_shape_semantics_summary.json`.
