...
- Compile probes to `sb/build/*.sb.bin` with `book.api.profile_tools`.
- Decode with `book.api.profile_tools.decoder` into joinable records under `out/` (node fields, tag, u16 role, literal refs).
