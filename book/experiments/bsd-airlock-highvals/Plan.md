...
- Compile probes to `sb/build/*.sb.bin` with `book.api.profile`.
- Decode with `book.api.profile.decoder` into joinable records under `out/` (node fields, tag, u16 role, literal refs).
