...
- `book/graph/concepts/validation/golden_corpus/corpus_manifest.json` – blob IDs, source paths, SHA-256, size, and category.
- `book/graph/concepts/validation/golden_corpus/raw/` – header/section snapshots for each blob.
- `book/graph/concepts/validation/golden_corpus/decodes/` – `decode_profile_dict` outputs per blob.
- `book/graph/concepts/validation/golden_corpus/inspect/` – `book.api.profile` inspect and op-table summaries per blob.
- `book/graph/concepts/validation/golden_corpus/corpus_summary.json` – key signals (op_count, node bytes, literal start, tag histogram, layout digest) consolidated per blob.
...
- Added validation job `experiment:golden-corpus` under `book/graph/concepts/validation/` to replay decoder/profile against the manifest; current status `ok` after normalizing tag-count comparisons.
...
- `book/graph/concepts/validation/golden_corpus/corpus_summary.json` – per-blob decoder vs profile signals (op_count, node bytes, literal start, tag histograms).
...
- `book/graph/concepts/validation/golden_corpus/inspect/*_inspect.json` and `*_op_table.json` – profile summaries.
- `book/graph/concepts/validation/fixtures/blobs/airlock.sb.bin` – static-only platform profile fixture for decoder/inspect use.
...
- `platform_airlock` uses the fixture blob `book/graph/concepts/validation/fixtures/blobs/airlock.sb.bin` (compiled from `/System/Library/Sandbox/Profiles/airlock.sb`).
