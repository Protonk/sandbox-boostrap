...
- Added `harvest_filters.py` to parse `_filter_info` and recover the Filter Vocabulary (masked pointers → `__TEXT.__cstring`). Harvest output: 93 filter names, first `path`, last `kas-info-selector`. Updated `book/graph/mappings/vocab/filters.json` to `status: ok` with these IDs.

## Filter names harvest

- Guardrail coverage now lives under `book/tests/` and mapping validation; avoid experiment-local “check” scripts for canonical artifacts.
