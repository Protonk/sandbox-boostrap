...
- Updated `harvest_field2.py` to treat the third slot explicitly as `filter_arg_raw` with derived `field2_hi = raw & 0xC000` and `field2_lo = raw & 0x3FFF`, and to track per-tag counts. Inventory now ingests `book/experiments/field2-final-final/probe-op-structure/sb/build` profiles alongside the local probes and system blobs; refreshed output lives at `out/field2_inventory.json`.
