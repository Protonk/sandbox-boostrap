...
- Baseline decode of canonical system profiles (`airlock`, `bsd`, `sample`) via `book.api.profile_tools.decoder`; wrote `out/tag_histogram.json` capturing tag counts and literal counts plus sections. Source blobs: `book/examples/extract_sbs/build/profiles/{airlock,bsd}.sb.bin`, `book/examples/sb/build/sample.sb.bin`.
