...
- Baseline decode of canonical system profiles (`airlock`, `bsd`, `sample`) via `book.api.profile_tools.decoder`; wrote `out/tag_histogram.json` capturing tag counts and literal counts plus sections. Source blobs: `book/graph/concepts/validation/fixtures/blobs/{airlock,bsd,sample}.sb.bin`.
