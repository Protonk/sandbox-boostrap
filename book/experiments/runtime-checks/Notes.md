...
- Ran the runtime_tools clean channel with staging to `/private/tmp` to avoid Desktop TCC; outputs synced back to `out/` with decision-stage results and sandbox_check callouts recorded in `runtime_results.json`.
