...
book/experiments/field2-atlas/Report.md:30:- Field2 1 (`mount-relative-path`): Anchored via `/etc/hosts` and present in `sys:sample` tag 8; runtime scenario `adv:path_edges:allow-subpath` is deny where expected allow. The mismatch is captured as a packet in `book/experiments/runtime-adversarial/out/mismatch_packets.jsonl` with baseline/oracle/normalization controls and labeled `path_normalization_sensitivity`.
