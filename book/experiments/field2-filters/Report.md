...
Second, the decoder now exposes structure with provenance rather than silently guessing. `book/api/profile_tools/decoder.py` remains permissive, but it now attaches `filter_arg_raw`, `u16_role`, and (when applicable) `filter_vocab_ref` / `filter_out_of_vocab` directly on decoded nodes, and it records provenance for layout selection and literal reference recovery. The decoder does not enforce “unknown-but-bounded”; discovery stays possible.
