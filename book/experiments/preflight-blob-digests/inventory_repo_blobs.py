...
def _bucket_for(path: Path) -> str:
    rel = _rel(path)
    if rel.startswith("book/graph/concepts/validation/fixtures/blobs/"):
        return "validation_fixtures"
    if rel.startswith("book/graph/concepts/validation/out/experiments/"):
        return "validation_out"
    if rel.startswith("book/graph/mappings/"):
        return "mappings"
    if rel.startswith("book/profiles/"):
        return "profiles"
    if rel.startswith("book/experiments/"):
        return "experiments"
