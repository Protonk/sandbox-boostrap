...
- **Done**: Built compile matrix in `sb/` (singles, pairs, triples; ordering permutations; `require-all`/`require-any`; nested triple; TCP/UDP variants; negative controls on `mach-lookup`).
  - Compiled via `python -m book.api.profile compile book/experiments/field2-final-final/flow-divert-2560/sb/*.sb --out-dir book/experiments/field2-final-final/flow-divert-2560/sb/build`.
