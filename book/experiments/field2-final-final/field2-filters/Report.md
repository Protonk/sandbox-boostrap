...
`flow-divert` probes: `filter_arg_raw=2560` (`0x0a00`) and `2816` (`0x0b00`) are now treated as characterized (triple-only) and are excluded from the “unknown” census; see `book/experiments/field2-final-final/flow-divert-2560/Report.md`. Other out-of-vocab values surfaced in the same probe family (for example `12096`) remain in the unknown set.
