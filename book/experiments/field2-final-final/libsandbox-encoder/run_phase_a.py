...
def load_tag_layouts() -> Dict[int, Dict[str, Any]]:
    ...
    for path in [base, ROOT / "book/experiments/field2-final-final/libsandbox-encoder/out/tag_layout_overrides.json"]:
        if not path.exists():
            continue
        try:
            data = json.loads(path.read_text())
...
def main() -> None:
    for stem in ["matrix_v1", "matrix_v2"]:
        sb_path = ROOT / f"book/experiments/field2-final-final/libsandbox-encoder/sb/{stem}.sb"
        if not sb_path.exists():
            continue
        out_blob = ROOT / f"book/experiments/field2-final-final/libsandbox-encoder/out/{stem}.sb.bin"
        out_json = ROOT / f"book/experiments/field2-final-final/libsandbox-encoder/out/{stem}_field2_encoder_matrix.json"
