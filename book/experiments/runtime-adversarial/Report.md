...
- Harness: `book.api.runtime.execution.harness.runner.run_matrix` + runtime-checks shims; file probes run via `sandbox_runner` + `file_probe` so each scenario applies exactly once inside a fresh worker; compile/decode via `book.api.profile` and `book.api.profile.decoder`.
