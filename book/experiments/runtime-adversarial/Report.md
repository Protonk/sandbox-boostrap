...
- Baseline comparator results live in `out/LATEST/baseline_results.json` (unsandboxed F_GETPATH path observations + loopback connect control for flow-divert probes).
- Decision-stage outcomes are present in `out/LATEST/runtime_results.json`; mismatches are restricted to the structural + path families (unexpected denies plus the known `/tmp`→`/private/tmp` normalization boundary).
...
- Mismatches emit bounded `out/LATEST/mismatch_packets.jsonl` packets with baseline/oracle/normalization controls and an allowlisted `mismatch_reason` (the `adv:path_edges:allow-subpath` mismatch is labeled `path_normalization_sensitivity`).
...
- Behavioral: the same test checks `adv:net_outbound_allow*` probes all yield allow and `adv:net_outbound_deny*` probes all yield deny in `out/LATEST/runtime_results.json`.
