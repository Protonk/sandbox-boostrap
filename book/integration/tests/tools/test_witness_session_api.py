...
@pytest.mark.system
def test_witness_xpc_session_multi_probe(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    assert WITNESS_CLI.exists(), "missing PolicyWitness CLI (book/tools/witness/PolicyWitness.app)"

    monkeypatch.setattr(witness_observer, "OBSERVER_MODE", "disabled")
    correlation_id = "test-witness-session"
    try:
        session = open_session(
            profile_id="minimal",
            plan_id="test:witness:session",
