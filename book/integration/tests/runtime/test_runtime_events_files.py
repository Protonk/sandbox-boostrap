...
def test_normalized_runtime_event_files_marker_free_and_world_stamped():
    ...
    event_sources = [
        ...
        (
            "metadata-runner",
            load_bundle_json(
                ROOT / "book" / "evidence" / "experiments" / "runtime-final-final" / "suites" / "metadata-runner" / "out",
                "runtime_events.normalized.json",
            ),
        ),
        (
            ...
            load_bundle_json(
                ROOT / "book" / "evidence" / "experiments" / "runtime-final-final" / "suites" / "vfs-canonicalization" / "out",
                "runtime_events.normalized.json",
            ),
        ),
    ]
