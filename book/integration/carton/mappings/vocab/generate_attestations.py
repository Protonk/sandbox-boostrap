...
def run_validation_job(job_id: str) -> None:
    ...
    lib_path = REPO_ROOT / "book/integration/carton/bundle/relationships/mappings/dyld-libs/usr/lib/libsandbox.1.dylib"
    if lib_path.exists():
...
def main() -> None:
    ...
    manifest = {
        ...
        "ops": {
            ...
            "hash": sha256(REPO_ROOT / "book/integration/carton/bundle/relationships/mappings/vocab/ops.json"),
            "sources": ops_src,
        },
        "filters": {
            "count": len(filters.get("filters", [])),
            "source": filters.get("filters", [{}])[0].get("source"),
            "hash": sha256(REPO_ROOT / "book/integration/carton/bundle/relationships/mappings/vocab/filters.json"),
            "sources": filt_src,
        },
