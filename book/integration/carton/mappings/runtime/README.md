...
Runtime probe outputs that are stable enough to reuse live under `book/integration/carton/bundle/relationships/mappings/runtime/`. These are versioned to the Sonoma baseline (23E224) and come from runtime promotion packets (clean launchd channel).
...
- `runtime_signatures.json` — small IR derived from promotion packets plus `field2_ir.json`, summarizing probe outcomes by profile plus a field2 summary; regenerated via `book/integration/carton/mappings/runtime/promote_from_packets.py` and (when promoted) included in the CARTON bundle (`book/integration/carton/bundle/`).
...
- `other_runtime_inventory.json` — normalized inventory of runtime-related repo paths used by hardened-runtime guardrails.
...
- Clean-channel runs emit promotion packets (`book/evidence/experiments/*/out/promotion_packet.json`), and generators refuse to promote decision-stage artifacts unless `channel=launchd_clean`.
...
- Use `book/integration/carton/bundle/relationships/mappings/vocab/ops_coverage.json` and explicit status fields to gauge what is currently runtime-backed vs blocked on this host.
...
- Run `book/integration/carton/mappings/runtime/promote_from_packets.py` (defaults to `packet_set.json`) to rebuild `book/integration/carton/bundle/relationships/mappings/runtime_cuts/`, `runtime_story`, `runtime_coverage`, `runtime_callout_oracle`, and `runtime_signatures.json` from promotion packets, and to refresh `promotion_receipt.json`.
- `op_runtime_summary.json` is regenerated by the same promotion path; see `book/integration/carton/mappings/runtime/generate_op_runtime_summary.py` for the standalone entrypoint.
- `runtime_links.json` is regenerated by the same promotion path; see `book/integration/carton/mappings/runtime/generate_runtime_links.py` for the standalone entrypoint.
- Rerun `book/integration/carton/mappings/runtime/generate_lifecycle.py` after updating lifecycle probes in `book/evidence/graph/concepts/validation/out/lifecycle/` to refresh `lifecycle.json` and `lifecycle_traces/`.
- `other_runtime_inventory.json` is normalized from `book/evidence/experiments/runtime-final-final/suites/hardened-runtime/other_runtime_inventory.json` via `book/integration/carton/mappings/runtime/generate_other_runtime_inventory.py`.
