...
Clusters: Semantic Graph and Evaluation
...
* **Validation pattern:**
  ...
  * Compile a parameterized specimen without params and assert a deterministic error; compile with params and confirm the parameter value is embedded in the literal table.
  * Compile a gated param specimen with multiple values and confirm the compiled blob is identical across values on this host (presence vs missing), then record the matrix for regressions.
* **Related concepts:** SBPL Profile; Regex / Literal Table; Policy Lifecycle Stage; Binary Profile Header; Compiled Profile Source.

Clusters: Semantic Graph and Evaluation; Runtime Lifecycle and Extension
...
* **Validation pattern:**
  ...
  * Use op-table vocab alignment outputs to confirm that operation names map to the expected IDs for compiled profiles on this host.
* **Related concepts:** Filter; Policy Node; PolicyGraph; Operation Vocabulary Map; Decision.

Clusters: Semantic Graph and Evaluation; Vocabulary and Mapping
...
* **Validation pattern:**
  ...
  * Cross-check filter nodes against the filter vocab map and tag-layout roles, recording out-of-vocab filter IDs and fallback layouts.
...
* **Validation pattern:**
  ...
  * Use the `runtime:metafilter_any` profile in runtime-checks to assert `require-any` semantics in normalized runtime outcomes.
...
* **Validation pattern:**
  ...
  * Use normalized runtime-checks outcomes (sys profiles, bucket profiles, metafilter cases) to confirm allow/deny decisions and detect apply/preflight gating.
...
* **Validation pattern:**
  ...
  * Verify platform-layer profiles via system-profile digests/static checks and confirm runtime-checks outcomes for `sys:airlock` and `sys:bsd`.
* **Related concepts:** Policy Stack Evaluation Order; Compiled Profile Source; Policy Lifecycle Stage; Sandbox Extension.

Clusters: Runtime Lifecycle and Extension
...
* **Validation pattern:**
  ...
  * Run the lifecycle extensions probe or PolicyWitness sandbox_extension flow to validate issue/consume/release behavior (expecting EPERM without entitlements).
...
* **Validation pattern:**
  ...
  * Use sandbox-init-params validation summaries and apply-attempt/gate-witness probes to pin the compile vs apply boundary on this host.
  * Maintain an apply-gated blob digest corpus to keep preflight and apply-stage gating detection stable.
...
* **Validation pattern:**
  ...
  * Verify canonical header words, op-count fields, and blob lengths against the header contract for system profiles.
* **Related concepts:** Profile Format Variant; Operation Pointer Table; Regex / Literal Table; PolicyGraph.

Clusters: Static-Format
...
* **Validation pattern:**
  ...
  * Use decoder validation (op-table scaling witness, stride coverage) to ensure op-table roots fall within the decoded node array on canonical blobs.
...
* **Validation pattern:**
  ...
  * Verify tag roles/layouts and node remainder contracts on the canonical corpus to ensure node decoding is aligned with expected layouts.
...
* **Validation pattern:**
  ...
  * Ingest compiled SBPL samples into graph IR and confirm expected profiles and section lengths are present in the ingested output.
* **Related concepts:** Policy Node; Operation Pointer Table; Decision; Policy Lifecycle Stage; Profile Format Variant.

Clusters: Semantic Graph and Evaluation; Static-Format
...
* **Validation pattern:**
  ...
  * Use decoder validation to confirm literal strings and literal references are present, and that parameter values are embedded in literal tables when params are provided.
...
* **Validation pattern:**
  ...
  * Record format_variant in system-profile static checks and golden-corpus replays to spot drift across canonical blobs.
...
Clusters: Static-Format; Vocabulary and Mapping
...
* **Validation pattern:**
  ...
  * Harvest operation names directly from the trimmed libsandbox dylib and assert parity with the canonical ops mapping.
* **Related concepts:** Operation; Operation Pointer Table; Profile Format Variant; Filter Vocabulary Map.

Clusters: Vocabulary and Mapping
...
* **Validation pattern:**
  ...
  * Harvest filter names from the trimmed libsandbox dylib and assert parity with the canonical filters mapping, tracking out-of-vocab filter IDs in tag-role/layout validation.
...
* **Validation pattern:**
  ...
  * Use platform-policy probes and sys-profile runtime outcomes to show platform-layer denies persist even when app-layer rules would allow.
...
* **Validation pattern:**
  ...
  * Use system-profile digest normalization and golden-corpus replays to pin canonical blobs to their sources and keep decoder summaries aligned.
