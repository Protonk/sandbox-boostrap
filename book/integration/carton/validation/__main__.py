...
Usage examples:
- python -m book.integration.carton validate --all
- python -m book.integration.carton validate --tag vocab
- python -m book.integration.carton validate --experiment field2
"""
...
METADATA_PATH = ROOT / "book" / "evidence" / "syncretic" / "validation" / "out" / "metadata.json"
...
def run_jobs(
    *,
    ...
    results = [
        run_job(job, skip_missing_inputs, host_meta, prev_status.get(job.id))
        for job in selected
    ]
