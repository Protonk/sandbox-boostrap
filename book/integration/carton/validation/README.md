...
- Run by tag/experiment: `python -m book.integration.carton validate --tag vocab` or `--experiment field2`
- Describe a job: `python -m book.integration.carton validate --describe <job_id>`
...
- Meta check: `python -m book.integration.carton validate --tag meta` runs `validation:schema-check` to assert status files follow this schema.
...
Smoke tag:
- `python -m book.integration.carton validate --tag smoke` runs the core fast jobs (vocab:sonoma-14.4.1 + experiment:field2 + experiment:runtime-checks) as a default pre-promotion gate. `tag:golden` marks canonical jobs.
