...
from book.integration.carton import build_manifest as build_mod

SCHEMA_DIR = Path("book/integration/carton/schemas")
...
def _contract_drift(repo_root: Path, world_id: str) -> List[str]:
    errors: List[str] = []
    expected = build_mod.build_contracts(repo_root=repo_root, world_id=world_id)
    for rel_path, doc in expected.items():
        ...
        if build_mod._sha256_canonical_json(current) != build_mod._sha256_canonical_json(doc):
            errors.append(f"contract drift: {rel_path} (regenerate contracts)")
    return errors


def _concept_index_paths_manifested(repo_root: Path, manifest_paths: set[str]) -> List[str]:
    errors: List[str] = []
    concept_index_path = repo_root / "book/graph/mappings/carton/concept_index.json"
    if not concept_index_path.exists():
        return ["concept_index.json missing"]
    doc = _load_json(concept_index_path)
    for entries in (doc.get("concepts") or {}).values():
        for entry in entries:
            path = entry.get("path")
            if not path:
                errors.append("concept_index entry missing path")
                continue
            if path not in manifest_paths:
                errors.append(f"concept_index path not in manifest: {path}")
            if not (repo_root / path).exists():
                errors.append(f"concept_index path missing on disk: {path}")
    return errors


def _invariant_statuses(repo_root: Path, spec: Dict[str, Any]) -> List[str]:
    errors: List[str] = []
...
def run_check(*, spec_path: Path, manifest_path: Path, repo_root: Path) -> List[str]:
    errors: List[str] = []
    spec = build_mod.load_spec(spec_path)
    ...
    if manifest.get("schema_version") != build_mod.MANIFEST_SCHEMA_VERSION:
        errors.append("manifest schema_version mismatch")
    spec_hash = build_mod._sha256_canonical_json(spec)
    ...
    for artifact_id, spec_entry in spec_entries.items():
        ...
        if digest_mode == "bytes":
            digest = build_mod._sha256_file(abs_path)
            ...
            digest = build_mod._sha256_canonical_json(data)
            if digest != manifest_entry.get("digest"):
                errors.append(f"digest mismatch for {rel_path}")
            byte_digest = manifest_entry.get("byte_digest")
            if byte_digest and byte_digest != build_mod._sha256_file(abs_path):
    ...
    errors.extend(_concept_index_paths_manifested(repo_root, manifest_paths))
    errors.extend(_contract_drift(repo_root, baseline_world_id))
    errors.extend(_invariant_statuses(repo_root, spec))
