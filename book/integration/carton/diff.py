...
def _load_contracts_from_manifest(repo_root: Path, manifest: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    ...
    for entry in manifest.get("artifacts", []):
        path = entry.get("path")
        if not path or not path.startswith("book/integration/carton/contracts/"):
            continue
...
def _diff_contracts(
    ...
    vocab_path = "book/integration/carton/contracts/vocab.contract.json"
    profiles_path = "book/integration/carton/contracts/profiles.contract.json"
    coverage_path = "book/integration/carton/contracts/coverage.contract.json"
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.other:
        ...
        other_manifest = build_mod.build_manifest_doc(spec_path=spec_path, repo_root=repo_root)
        live_contracts = True
    ...
    if live_contracts:
        world_id, _ = build_mod._baseline_world(repo_root)
        other_contracts = build_mod.build_contracts(repo_root=repo_root, world_id=world_id)
    else:
